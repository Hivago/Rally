...
/// <summary>
/// One weekly ICICI bulk-transfer export for rider payouts. Mirrors
/// RallyAPI.Orders.Domain.Entities.RestaurantPayoutExportBatch — every
/// <see cref="RiderPayoutLedger"/> included in the export stamps its ExportBatchId to this
/// record's Id, so the file that went out, its control-sum, and its later reconciliation are
/// all traceable from one row (see specs/icici-manual-payout-export.md, section 4a).
/// </summary>
public sealed class RiderPayoutExportBatch : AggregateRoot
{
    ...
    // EF Core
    private RiderPayoutExportBatch() { }

    public static RiderPayoutExportBatch Create(
        ...
        string generatedFileHash)
    {
        ...
        return new RiderPayoutExportBatch
        {
            Id = Guid.NewGuid(),
